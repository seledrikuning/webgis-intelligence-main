...
define([
  ...
  function C(e, t, n, o, a, c, d, h, u, l, I, g, m, T) {
    const E = r.defined(d),
      f = u.north,
      p = u.south;
    let y = u.east;
    ...
    for (let r = 0; r < S; ++r) {
      ...
      o.hasWebMercatorT &&
        (C =
          (s.WebMercatorProjection.geodeticLatitudeToMercatorAngle(x.latitude) -
            I) *
          g),
  ...
  return c(function (c, d) {
    const h = c.quantizedVertices,
      u = h.length / 3,
      l = c.octEncodedNormals,
      I =
        c.westIndices.length +
        c.eastIndices.length +
        c.southIndices.length +
        c.northIndices.length,
      m = c.includeWebMercatorT,
      T = c.exaggeration,
      E = c.exaggerationRelativeHeight,
      f = 1 !== T,
      p = t.Rectangle.clone(c.rectangle),
      y = p.west,
      v = p.south,
      P = p.east,
      D = p.north,
      k = t.Ellipsoid.clone(c.ellipsoid),
      F = c.minimumHeight,
      H = c.maximumHeight,
      _ = c.relativeToCenter,
      G = a.Transforms.eastNorthUpToFixedFrame(_, k),
      V = t.Matrix4.inverseTransformation(G, new t.Matrix4());
    let Y, O;
    m &&
      ((Y = s.WebMercatorProjection.geodeticLatitudeToMercatorAngle(v)),
      (O =
        1 / (s.WebMercatorProjection.geodeticLatitudeToMercatorAngle(D) - Y)));
    ...
    for (let e = 0; e < u; ++e) {
      ...
      (U[e] = new t.Cartesian2(o, a)),
        ...
        m &&
          (Q[e] =
            (s.WebMercatorProjection.geodeticLatitudeToMercatorAngle(
              x.latitude
            ) -
              Y) *
            O),
    ...
    const re = W(c.westIndices, function (e, t) {
        return U[e].y - U[t].y;
      }),
      ne = W(c.eastIndices, function (e, t) {
        return U[t].y - U[e].y;
      }),
      oe = W(c.southIndices, function (e, t) {
        return U[t].x - U[e].x;
      }),
      ie = W(c.northIndices, function (e, t) {
        return U[e].x - U[t].x;
      });
    let ae;
    if (F < 0) {
    ...
    (se = Math.min(
      se,
      A(c.westIndices, c.westSkirtHeight, z, U, p, k, V, X, Z)
    )),
      (se = Math.min(
        se,
        A(c.southIndices, c.southSkirtHeight, z, U, p, k, V, X, Z)
      )),
      (se = Math.min(
        se,
        A(c.eastIndices, c.eastSkirtHeight, z, U, p, k, V, X, Z)
      )),
      (se = Math.min(
        se,
        A(c.northIndices, c.northSkirtHeight, z, U, p, k, V, X, Z)
      ));
    ...
    let Se = u * he;
    return (
      C(ue, Se, re, de, z, U, l, k, p, c.westSkirtHeight, Y, O, pe, 0),
      (Se += c.westIndices.length * he),
      C(ue, Se, oe, de, z, U, l, k, p, c.southSkirtHeight, Y, O, 0, we),
      (Se += c.southIndices.length * he),
      C(ue, Se, ne, de, z, U, l, k, p, c.eastSkirtHeight, Y, O, ye, 0),
      (Se += c.eastIndices.length * he),
      C(ue, Se, ie, de, z, U, l, k, p, c.northSkirtHeight, Y, O, 0, Ne),
      g.addSkirtIndices(re, oe, ne, ie, u, me, c.indices.length),
      d.push(ue.buffer, me.buffer),
      {
        ...
        encoding: de,
        indexCountWithoutSkirts: c.indices.length,
      }
    );
  });
});
