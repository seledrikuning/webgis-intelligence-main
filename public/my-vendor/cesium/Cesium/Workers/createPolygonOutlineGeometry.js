...
define([
  ...
  const H = [],
    T = [];
  function v(e, t, o, y, u) {
    const f = n.EllipsoidTangentPlane.fromPoints(t, e).projectPointsOntoPlane(
      t,
      H
    );
    let g, m;
    c.PolygonPipeline.computeWindingOrder2D(f) === c.WindingOrder.CLOCKWISE &&
      (f.reverse(), (t = t.slice().reverse()));
    let h = t.length,
      b = 0;
    if (y)
      ...
      if (u === i.ArcType.GEODESIC)
        for (m = 0; m < h; m++)
          r += d.PolygonGeometryLibrary.subdivideLineCount(
            t[m],
            t[(m + 1) % h],
            o
          );
        ...
        for (m = 0; m < h; m++)
          r += d.PolygonGeometryLibrary.subdivideRhumbLineCount(
            e,
            t[m],
            t[(m + 1) % h],
            o
      ...
      for (g = new Float64Array(3 * r), m = 0; m < h; m++) {
        ...
        u === i.ArcType.GEODESIC
          ? (r = d.PolygonGeometryLibrary.subdivideLine(
              t[m],
              t[(m + 1) % h],
              o,
              T
          ...
          : u === i.ArcType.RHUMB &&
            (r = d.PolygonGeometryLibrary.subdivideRhumbLine(
              e,
              t[m],
              t[(m + 1) % h],
              o,
  ...
  function O(o) {
    const n = o.polygonHierarchy,
      a = e.defaultValue(o.ellipsoid, t.Ellipsoid.WGS84),
      s = e.defaultValue(o.granularity, r.CesiumMath.RADIANS_PER_DEGREE),
    ...
    let p = e.defaultValue(o.height, 0),
      c = e.defaultValue(o.extrudedHeight, p);
    if (!y) {
      const e = Math.max(p, c);
      (c = Math.min(p, c)), (p = e);
    }
    (this._ellipsoid = t.Ellipsoid.clone(a)),
      ...
      (this.packedLength =
        d.PolygonGeometryLibrary.computeHierarchyPackedLength(n, t.Cartesian3) +
  ...
  O.pack = function (i, o, r) {
    return (
      ...
      (r = d.PolygonGeometryLibrary.packPolygonHierarchy(
        i._polygonHierarchy,
        o,
        r,
        t.Cartesian3
  ...
  return (
    (O.unpack = function (i, o, r) {
      ...
      const n = d.PolygonGeometryLibrary.unpackPolygonHierarchy(
        i,
        o,
        t.Cartesian3
      );
    ...
    (O.createGeometry = function (t) {
      const i = t._ellipsoid,
        ...
        p = t._arcType,
        f = d.PolygonGeometryLibrary.polygonOutlinesFromHierarchy(s, !l, i);
      if (0 === f.length) return;
      let g;
      const m = [],
        h = r.CesiumMath.chordLength(n, i.maximumRadius),
        b = t._height,
        P = t._extrudedHeight;
      let E, A;
      if (
        t._perPositionHeightExtrude ||
        !r.CesiumMath.equalsEpsilon(b, P, 0, r.CesiumMath.EPSILON2)
      )
            ...
            t._offsetAttribute === y.GeometryOffsetAttribute.TOP
              ...
              : ((E =
                  t._offsetAttribute === y.GeometryOffsetAttribute.NONE
                    ...
                    : 1),
                (i = i.fill(E))),
              (g.geometry.attributes.applyOffset = new a.GeometryAttribute({
      ...
      const _ = u.GeometryPipeline.combineInstances(m)[0],
