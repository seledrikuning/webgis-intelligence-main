  ...
  function r(t, i, r, h) {
    a.Cartesian3.normalize(h.cartographicToCartesian(i, o), s),
      ...
      (function (t, a, i, s, o, r, h) {
        ...
        const y = (q * (a * a - i * i)) / (i * i),
          x = (y * (256 + y * (y * (74 - 47 * y) - 128))) / 1024,
          D = U * U,
          E =
            i *
                ...
                (U +
                  (x *
                    (O * (2 * D - 1) -
                      (x * U * (4 * S * S - 3) * (4 * D - 3)) / 6)) /
                    4)),
          P = Math.atan2(p * R, m - H * w),
          T = Math.atan2(M * R, m * w - H);
        (t._distance = E),
          (t._startHeading = P),
          (t._endHeading = T),
          (t._uSquared = y);
      ...
      let H =
        ...
        o *
          (1 -
            ...
            ((5 * m) / 4 - (115 * _) / 16) * h -
            (29 * _ * d) / 16) +
        (f / 2 - C + (71 * m) / 32 - (85 * _) / 16) * c +
        ((5 * C) / 16 - (5 * m) / 4 + (383 * _) / 96) * u -
        g * ((m - (11 * _) / 2) * c + (5 * _ * u) / 2) +
        ((29 * m) / 96 - (29 * _) / 16) * l +
        (539 * _ * M) / 1536;
      const v = Math.asin(Math.sin(H) * s.cosineAlpha),
        O = Math.atan((s.a / s.b) * Math.tan(v));
      H -= s.sigma;
      const S = Math.cos(2 * s.sigma + H),
        q = Math.sin(H),
        U = Math.cos(H),
        A = s.cosineU * U,
        b = s.sineU * q,
        w =
          Math.atan2(q * s.sineHeading, A - b * s.cosineHeading) -
          e(s.f, s.sineAlpha, s.cosineSquaredAlpha, H, q, U, S);
      return i.defined(n)
